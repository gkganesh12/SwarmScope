...
Runs simulation in background, records each Agent's actions, supports real-time status monitoring
"""
...
logger = get_logger('swarmscope.simulation_runner')
...
class SimulationRunState:
    ...
    # Per-platform independent rounds and sim time (for dual-platform parallel display)
    ...
    def add_action(self, action: AgentAction):
        """Add action to recent action list"""
...
class SimulationRunner:
    ...
    3. Provide real-time status query API
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Max simulation rounds (Options, truncates overly long simulations)
        ...
        Args:
            ...
            max_rounds: Max simulation rounds (Options, truncates overly long simulations)
        ...
        try:
            ...
            # Create main log file to avoid stdout/stderr pipe buffer overflow blocking process
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determine if platform was enabled by checking if corresponding actions.jsonl file exists
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        if IS_WINDOWS:
            ...
            try:
                ...
                logger.warning(f"Process group not responding to SIGTERM, force terminate: {simulation_id}")
    ...
    def stop_simulation(cls, simulation_id: str) -> SimulationRunState:
        ...
        if process and process.poll() is None:
            try:
                ...
                logger.error(f"Terminating process groupfailed: {simulation_id}, error={e}")
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check if there is content to clean up (avoid empty processes printing useless logs)
    ...
    def register_cleanup(cls):
        ...
        Called on Flask app startup to ensure cleanup of all simulation processes on server shutdown
        """
        ...
        # In Flask debug mode, only register cleanup in reloader subprocess (the actual app process)
        # WERKZEUG_RUN_MAIN=true Indicates the reloader subprocess
        # If not in debug mode, this env var is absent, still need to register
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (Options)
                - "twitter": Only interview on Twitter platform
                - "reddit": Only interview on Reddit platform
                - None: In dual-platform simulation, interview on both platforms simultaneously, return integrated results
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (Options, overridden by each interview item's platform)
                - "twitter": Default: only interview on Twitter platform
                - "reddit": Default: only interview on Reddit platform
                - None: For dual-platform simulation, interview each Agent on both platforms simultaneously
            timeout: Timeout (seconds)
    ...
    def get_interview_history(
        ...
        Args:
            ...
            agent_id: Specified Agent ID (Options, only get this Agent's history)
