...
Improvements:
1. Use Zep search to enrich node information
...
logger = get_logger('swarmscope.oasis_profile')
...
class OasisProfileGenerator:
    ...
    Convert Zep graph entities to Agent Profiles needed for OASIS simulation
    
    Optimized features:
    1. Use Zep graph search to get richer context
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        comprehensive_query = f"About{entity_name}'s all information, activities, events, relationships and background"
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        logger.debug(f"ZepEdge search attempt {attempt + 1}  failed: {str(e)[:80]}, retrying...")
                        time.sleep(delay)
                        delay *= 2
                    else:
                        logger.debug(f"ZepEdge search after {max_retries}  attempts still failed: {e}")
            return None
        ...
        def search_nodes():
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    last_exception = e
                    if attempt < max_retries - 1:
                        logger.debug(f"ZepNode search attempt {attempt + 1}  failed: {str(e)[:80]}, retrying...")
                        time.sleep(delay)
                        delay *= 2
                    else:
                        logger.debug(f"ZepNode search after {max_retries}  attempts still failed: {e}")
            return None
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    # No max_tokens, let LLM decide output length
                )
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        # 7. Complete failure, returning basic structure
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic personas for opinion simulation, maximally faithful to existing real-world conditions. Must return valid JSON format, all string values must not contain unescaped newlines. Use the original language of the source material."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        context_str = context[:3000] if context else "NoneAdditional context"
        
        return f"""Generate detailed social media user persona for entity, maximally faithful to existing real-world conditions.
...
2. persona: Detailed persona description (2000 words plain text), should include:
   ...
   - Stance and viewpoints (attitude toward topics, content that may anger/move them)
...
- Use the language matching the source material (except gender field must be English male/female)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate detailed social media account settings for institution/group entity, maximally faithful to existing real-world conditions.
...
2. persona: Detailed account settings description (2000 words plain text), should include:
   ...
   - Stance and attitude (official stance on core topics, approach to controversies)
...
- Use the language matching the source material (except gender field must be English "other")
...
- Institutional account speech must match its identity positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        def save_profiles_realtime():
            """Save generated profiles to file"""
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            ...
            try:
                ...
                logger.error(f"generating entity {entity.name} 's persona failed: {str(e)}")
        ...
        print(f"\n{'='*60}")
        print(f"Starting agent persona generation - total {total} entities, parallel workers: {parallel_count}")
        print(f"{'='*60}\n")
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        Use same format as to_reddit_format() to ensure OASIS can read correctly.
