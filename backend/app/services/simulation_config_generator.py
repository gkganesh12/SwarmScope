...
Uses step-by-step generation to avoid failure from overly long output:
...
logger = get_logger('swarmscope.simulation_config')

# Chinese daily routine config (Beijing time)
...
class SimulationConfigGenerator:
    ...
    and auto-generate optimal simulation parameters
    ...
    def generate_config(
        ...
        # 1. Build base context info (context is kept in Chinese as it's sent to Chinese-language LLM)
...
### Basic principles (for reference only, adjust based on event nature and participant demographics):
- Target users are Chinese, config should follow Beijing time daily routines
...
- 19-22 PM: peak hours (activity coefficient 1.5)
- After 23 PM: declining activity (activity coefficient 0.5)
- General pattern: low at dawn, increasing in morning, moderate during work, peak in evening
- **Important**: The example values below are for reference only. You should adjust based on event nature and participant characteristics.
...
- reasoning (string): Brief explanation of why this config was chosen"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format. Time config should follow Chinese daily routines."
    ...
    def _get_default_time_config(self, num_entities: int) -> Dict[str, Any]:
        """Get default time config (Chinese daily routine)"""
        return {
            "total_simulation_hours": 72,
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        Match the best agent_id for each post based on poster_type
        """
...
- **Schedule follows Chinese daily routines**: Almost no activity 0-5 AM, most active 19-22 PM
...
{{
    "agent_configs": [
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON. Config should follow Chinese daily routines."
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        if entity_type in ["university", "governmentagency", "ngo"]:
            ...
            # Expert/professor: work+evening, medium frequency
            return {
                "activity_level": 0.4,
                "posts_per_hour": 0.3,
