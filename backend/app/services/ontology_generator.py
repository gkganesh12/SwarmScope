...
A. **Fallback types (must be included, placed last in the list)**:
...
- Various characters appear in text, such as "elementary school teachers", "passersby", "anonymous netizens"
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        # Truncate if over 50k chars (only affects LLM input, not graph building)
        if len(combined_text) > self.MAX_TEXT_LENGTH_FOR_LLM:
            combined_text = combined_text[:self.MAX_TEXT_LENGTH_FOR_LLM]
            combined_text += f"\n\n...(original text is {original_length} chars, truncated to first {self.MAX_TEXT_LENGTH_FOR_LLM} chars for ontology analysis)..."
...
"""
    ...
    def generate_python_code(self, ontology: Dict[str, Any]) -> str:
        ...
        code_lines = [
            ...
            'Auto-generated by SwarmScope for social opinion simulation',
            '"""',
            '',
