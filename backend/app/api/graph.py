...
# Get logger
logger = get_logger('swarmscope.api')
...
def generate_ontology():
    """
    API 1: Upload files, analyze and generate ontology definitions
...
def build_graph():
    ...
    try:
        ...
        if project.status == ProjectStatus.CREATED:
            return jsonify({
                "success": False,
                "error": "Project ontology not yet generated, call /ontology/generate first"
            }), 400
        ...
        # Get config
        graph_name = data.get('graph_name', project.name or 'SwarmScope Graph')
        ...
        def build_task():
            build_logger = get_logger('swarmscope.build')
            try:
