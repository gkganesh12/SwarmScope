...
logger = get_logger('swarmscope.api.simulation')
...
def get_graph_entities(graph_id: str):
    ...
    QueryParameters：
        entity_types: Comma-separated entity type list (Options, for further filtering)
    ...
    Request (JSON):
        {
            ...
            "graph_id": "swarmscope_xxxx",    // Optional, obtained from project if not provided
...
def prepare_simulation():
    ...
    try:
        ...
        if not force_regenerate:
            logger.debug(f"Checking simulation {simulation_id} whether preparation is complete...")
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Completed preparation exists, no need to regenerate",
...
def get_prepare_status():
    ...
    2. Check via simulation_id if completed preparation exists
...
def list_simulations():
    ...
    QueryParameters：
        project_id: Filter by project ID (Options)
    """
    try:
...
def get_simulation_history():
    ...
    For homepage historical project display, return simulation list with rich info like project name, description
    
    QueryParameters：
        limit: Limit on results returned (default 20)
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    - Return extra metadata (e.g., file modification time, whether generation is in progress)
...
def generate_profiles():
    ...
    Request (JSON):
        {
            "graph_id": "swarmscope_xxxx",     // Required
...
def start_simulation():
    ...
    About enable_graph_memory_update:
        - When enabled, all Agent activities in simulation (posting, commenting, liking, etc.) are updated to Zep graph in real-time
        - This lets the graph "remember" the simulation process for subsequent analysis or AI chat
            ...
            if is_prepared:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            try:
                                ...
                                logger.warning(f"Warning occurred while stopping simulation: {str(e)}")
                        else:
                            return jsonify({
                                "success": False,
                ...
                if force:
                    ...
                    if not cleanup_result.get("success"):
                        logger.warning(f"Warning occurred while cleaning logs: {cleanup_result.get('errors')}")
...
def get_run_status(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Get run statefailed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_run_status_detail(simulation_id: str):
    ...
    QueryParameters：
        platform: Platform filter（twitter/reddit，Options）
...
def get_simulation_timeline(simulation_id: str):
    ...
    returnedSummary info per round
    """
    try:
...
def get_simulation_comments(simulation_id: str):
    ...
    QueryParameters：
        post_id: Filter by post ID (Options)
...
def interview_agent():
    ...
    Note: This feature requires simulation environment to be running (enters command mode after completing simulation loop)
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment not running or closed. Please ensure simulation completed and entered command mode."
            }), 400
            ...
            "interviews": [                    // Required，Interview list
                {
                    ...
                    "platform": "twitter"      // Options，Specify this Agent's interview platform
                },
                {
                    "agent_id": 1,
                    "prompt": "What is your view on B?"  // Use default value if platform not specified
                }
            ],
...
def close_simulation_env():
    ...
    Note: This differs from /stop endpoint, /stop force terminates the process,
