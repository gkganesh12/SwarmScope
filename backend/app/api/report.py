...
logger = get_logger('swarmscope.api.report')
...
def get_generate_status():
    ...
    try:
        ...
        # If simulation_id provided, first check for an existing completed report
        if simulation_id:
            existing_report = ReportManager.get_report_by_simulation(simulation_id)
            if existing_report and existing_report.status == ReportStatus.COMPLETED:
                return jsonify({
...
def get_agent_log(report_id: str):
    ...
    Retrieve each step during report generation in real time, including:
...
def get_console_log(report_id: str):
    ...
    Retrieve console output during report generation (INFO, WARNING, etc.) in real time.
