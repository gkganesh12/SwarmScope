...
# Fixes OASIS third-party libraries reading files without specifying encoding
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Reconfigure stdout to UTF-8 (fix console encoding issues)
    ...
    # Note: this must be set at Python startup; runtime changes may not take effect
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        """
        Wraps open() to use UTF-8 encoding by default for text mode
...
class MaxTokensWarningFilter(logging.Filter):
    """Filters out camel-ai max_tokens warnings (we intentionally omit max_tokens to let the model decide)"""
...
def fetch_new_actions_from_db(
    ...
    """
    Get new action records from database with full context info
    
    Args:
        db_path: Database file path
        last_rowid: Maximum rowid value from last read (using rowid instead of created_at due to format differences between platforms)
...
def _enrich_action_context(
    ...
    """
    Supplement context info for actions (post content, usernames, etc.)
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    If boost LLM is configured, different platforms can use different API providers during parallel simulation for better concurrency.
    ...
    # Choose which LLM to use based on params and config
...
async def run_twitter_simulation(
    ...
    Args:
        ...
        max_rounds: Maximum rounds (optional, truncates overly long simulations)
    ...
    log_info(f"Simulation loop completed! elapsed: {elapsed:.1f}seconds, total_actions: {total_actions}")
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Max simulation rounds (optional, truncates overly long simulations)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
    ...
    # Create shutdown event at start of main function，Ensure entire program can respond to exit signals
...
def setup_signal_handlers(loop=None):
    ...
    Persistent simulation: don't exit after completion, wait for interview commands
    When receiving termination signal:
    1. Notify asyncio loop to exit wait
    2. Gives the program a chance to cleanly release resources (close database, environment, etc.)
    3. Then exit
    """
